...
class ZabbixActionITest extends ZabbixTest {
  ...
  "Action" should {
    "create host with invalid group should throw exception" in {
      val cha = ActionFactory.get(ActionFactory.ACTION_CREATE_HOST)(hostname, Some(hostip),
    ...
    "delete host should be successful" in {
      val cha = ActionFactory.get(ActionFactory.ACTION_DELETE_HOST)(hostname, None, None, None, None)
      cha must beSome
      cha.get.execute()
